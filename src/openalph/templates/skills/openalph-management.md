...
| `/cache gc` | Apply a context-GC boundary now: tool outputs → pointers, thinking dropped, durable set re-attached (replaces `/cache toolstrip`). Kill switch: `context.handoff_enabled = false` in the agent TOML |
